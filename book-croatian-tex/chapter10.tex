...
dostupne su i \texttt{long long} inačice
...
bitovno optimizirana inačica trajala je samo $3.1$ sekundu
...
Svaki takav odabir daje potproblem
...
\begin{lstlisting}
for (int s = 1; s < (1<<n); s++) {
    // početna vrijednost: potrebno je n+1 vožnji
    best[s] = {n+1,0};
    for (int p = 0; p < n; p++) {
        if (s&(1<<p)) {
            auto option = best[s^(1<<p)];
            if (option.second+weight[p] <= x) {
                // dodaj p u postojeću vožnju
                option.second += weight[p];
            } else {
                // rezerviraj novu vožnju za p
                option.first++;
                option.second = weight[p];
            }
